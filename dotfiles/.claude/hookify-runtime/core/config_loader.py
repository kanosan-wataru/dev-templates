...
def load_rules(event: Optional[str] = None) -> List[Rule]:
    ...
    # Searches both cwd-relative .claude/ (project scope) and ~/.claude/ (user scope)
    cwd_pattern = os.path.join('.claude', 'hookify.*.local.md')
    user_pattern = os.path.join(os.path.expanduser('~'), '.claude', 'hookify.*.local.md')
    files = list(set(glob.glob(cwd_pattern) + glob.glob(user_pattern)))
